...
algo = (
    PPOConfig()
    .env_runners(num_env_runners=64,rollout_fragment_length=1)
    .training(train_batch_size=(128), lr_schedule = [[0,1e-6],[0,1e-6],[200000,1e-8]])
    .resources(num_cpus_per_worker=0.5)
    .environment(env= "ReactorEcom-v10")
    .evaluation(evaluation_interval = 10, evaluation_parallel_to_training=True, evaluation_num_env_runners=1, evaluation_config={"explore":False})
    .build()
)
