...
os.makedirs('/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/TestMonths/HERON', exist_ok=True)

for m in range(4,5,1):

    os.makedirs(f'/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/TestMonths/HERON/Month_{m}', exist_ok=True)

    os.chdir('/home/rigbac/Projects/ALPACA')

    os.system('cp -a -f /home/rigbac/Projects/ALPACA/HPC_runs/FileStore_DontDisturb/. /home/rigbac/Projects/ALPACA/HPC_runs/DSFinalStore6')
    ...
    for i in range(0,7,1):
        ...
        csv_file = f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/TestMonths/HERON/Month_{m}/output_month_{m}_HERON.csv"

        hour_start = i*120 + m*720

        csv_actions = f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/TestMonths/HERON/dispatch_print_730_45.csv"

        action_df= pd.read_csv(csv_actions)
        ...
        while not (terminated or truncated):
            ...
            power_boost = action_df.loc[action_df['Time'] == hour, 'Dispatch__cspturbine__production__electricityBoost'].values[0]

            print(power_boost)

            power_boost = float(power_boost) - 0.4

            print(power_boost)

            if 7.5<power_boost:
                action = 0
            elif 6.5<=power_boost<7.5:
                action = 1
            elif 5.5<=power_boost<6.5:
                action = 2
            elif 4.5<=power_boost<5.5:
                action = 3
            elif 3.5<=power_boost<4.5:
                action = 4
            elif 2.5<=power_boost<3.5:
                action = 5
            elif 1.5<=power_boost<2.5:
                action = 6
            else:
                action = 7


            print(f'Time = {hour},Power Boost = {power_boost} Action = {action}')
            #action = 3
            ...
            profit += average_power*(observation[-4]*116.32134)/1e6
            t+=3600
            hour+=1
        ...
        os.system('cp -a -f /scratch/rigbac/ThreadFilesFWHPT/' + str(tid) + '/dsres.mat /home/rigbac/Projects/ALPACA/HPC_runs/DSFinalStore6')
        os.system('cp -a -f /scratch/rigbac/ThreadFilesFWHPT/' + str(tid) + '/dsfinal.txt /home/rigbac/Projects/ALPACA/HPC_runs/DSFinalStore6')
        ...
        plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/TestMonths/HERON/Month_{m}/Iteration_year_{i}.png")
        plt.close()

        plt.plot(rewards)
        plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/TestMonths/HERON/Month_{m}/Rewards.png")
        plt.close()
