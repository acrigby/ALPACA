...
policy = Policy.from_checkpoint("/home/rigbac/Projects/ALPACA/HPC_runs/Output/Checkpoints/TrainedPPOEcom_new_3_2_950")
...
algo_config = (
    PPOConfig()
    .env_runners(num_env_runners=80,rollout_fragment_length=1)
    .training(train_batch_size=(1000), lr_schedule = [[0,1e-4], [200000, 1e-6]])
    .resources(num_cpus_per_worker=0.5)
    .environment(env= "ReactorEcom-v9")
    .build()
)
...
algo = (
    PPOConfig()
    .env_runners(num_env_runners=80,rollout_fragment_length=1)
    .training(train_batch_size=(1000), lr_schedule = [[0,1e-5],[200000,1e-4],[400000,1e-6]])
    .resources(num_cpus_per_worker=0.5)
    .environment(env= "ReactorEcom-v9")
    .build()
)
...
for i in range(3000):
    ...
    if i % 10 == 0:
        ...
        while not (terminated or truncated):
          ...
          prob_action, Prob_state, prob_extra = algo.compute_single_action(observation, full_fetch = True)
          print(f'Action = {action} : Prob = {extra[SampleBatch.ACTION_PROB]}')
          print(f'Probabalistic Action = {prob_action} : Prob = {prob_extra[SampleBatch.ACTION_PROB]}')
          next_observation, reward, terminated, truncated, results = env.step(action)
          times.append(t/(3600*24))
          times.append((t+3599)/(3600*24))
          price_list.append(next_observation[-5])
          price_list.append(next_observation[-5])
          score += (reward)*1000
          t+=3600
        ...
        plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/PPO_Ecom3_Retrain/Iteration_{i}.png")
        plt.close()

        plt.plot(rewards)
        plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/PPO_Ecom3_Retrain/Rewards.png")
        plt.close()
        # Print the final score
        print(f"FINAL SCORE: {score} - Temperature: {0}") # Print the final score
    
    if i % 10 == 0:
        checkpoint_dir = algo.save(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Checkpoints/TrainedPPOEcom_new_3_Retrain_{i}").checkpoint.path
