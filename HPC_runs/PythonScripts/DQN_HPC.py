...
algo = (
    DQNConfig().training(lr=0.0001,train_batch_size=(256))
    .env_runners(num_env_runners=32)
    .resources(num_cpus_per_worker=1,num_gpus=0)
    .environment(env= "Reactor-v9")
    .build()
)
...
for i in range(3000):
    ...
    if i % 5 == 0:
        ...
        plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/DQN/Iteration_{i}.png")
        plt.close()

        plt.plot(rewards)
        plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/DQN/Rewards.png")
        plt.close()
        # Print the final score
        print(f"FINAL SCORE: {score} - Temperature: {0}") # Print the final score
    
    if i % 50 == 0:
        checkpoint_dir = algo.save(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Checkpoints/TrainedDQN_{i}").checkpoint.path
