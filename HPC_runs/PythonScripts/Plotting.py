...
# Example usage
file_path = '/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/FunctionalPPO/output_year_functional.csv'
...
fig.suptitle(f'Profit = {profit:.4} USD' +f'  Profit per day = {ppd:.4} USD')


ax1[0].set_xlabel('time (s)')
ax1[0].set_ylabel('Power Production / MW')
ax1[0].plot(vals_dict['column_0'],vals_dict['column_1'], alpha=0.6)
ax1[0].tick_params(axis='y')
for time in vals_dict[f'times']:
    #print(time)
    ax1[0].axvspan(time, time+(1/24), color='orange', alpha=0.2, label = "Pressure Band")

ax2 = ax1[0].twinx()  # instantiate a second Axes that shares the same x-axis

color = 'tab:red'
ax2.set_ylabel('Concrete Temperature / degC')  # we already handled the x-label with ax1
ax2.plot(vals_dict['column_0'],vals_dict['column_3'], color=color)
...
plt.savefig('/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/FunctionalPPO/DispatchPlot.png')