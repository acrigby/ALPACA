...
algo = (
    ...
    .training(train_batch_size=(1000), lr_schedule = [[0,1e-4],[400000,1e-8]])
    .resources(num_cpus_per_worker=0.5)
    .environment(env= "ReactorEcom-v8")
    .build()
)
...
for i in range(3000):
    ...
    if i % 5 == 0:
        ...
        while not (terminated or truncated):
          ...
          price_list.append(next_observation[-4])
          price_list.append(next_observation[-4])
          score += (reward)*1000
          t+=3600
        ...
        axs[1,1].axhspan(166.5, 170.5, color='green', alpha=0.5, label = "Temperature Band")

        axs[0,1].set_title('Deaerator Pressure')
        axs[0,1].plot(ts, pressures, 'tab:red')
        axs[0,1].set(ylabel='Pressure / bar')
        axs[0,1].axhspan(1, 2, color='green', alpha=0.5, label = "Pressure Band")
        ...
        plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/PPO_Ecom3_5/Iteration_{i}.png")
        plt.close()

        plt.plot(rewards)
        plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/PPO_Ecom3_5/Rewards.png")
        plt.close()
        # Print the final score
        print(f"FINAL SCORE: {score} - Temperature: {0}") # Print the final score
    
    if i % 5 == 0:
        checkpoint_dir = algo.save(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Checkpoints/TrainedPPOEcom_new_3_5_{i}").checkpoint.path
