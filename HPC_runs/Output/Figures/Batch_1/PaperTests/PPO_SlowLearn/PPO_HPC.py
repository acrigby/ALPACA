...
algo = (
    PPOConfig()
    .env_runners(num_env_runners=60, rollout_fragment_length=1)
    .training(train_batch_size=(2048),lr_schedule=([0,5e-5],[400000,1e-5],[800000,1e-6]))
    .resources(num_cpus_per_worker=0.75,num_gpus=0)
    .environment(env= "Reactor-v8")
    .build()
)

#algo = Algorithm.from_checkpoint('/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/PaperTests/PPO/FinalControlPPO_100')
"""
...
for i in range(3000):
    ...
    if i % 25 == 0:
        ...
        plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/PaperTests/PPO_SlowLearn/Iteration_{i}.png")
        plt.close()

        plt.plot(rewards)
        plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/PaperTests/PPO_SlowLearn/Rewards.png")
        plt.close()
        # Print the final score
        print(f"FINAL SCORE: {score} - Temperature: {0}") # Print the final score
    
    if i % 50 == 0:
        checkpoint_dir = algo.save(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/PaperTests/PPO_SlowLearn/FinalControlPPO_{i}").checkpoint.path
