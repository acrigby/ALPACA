...
def getprofit(vals_dict,errors,agent_type,month):
    profit = 0
    days = 1/24
...
for month in range(0,12,1):
    ...
    fig, ax1 = plt.subplots(6,1,sharex='all', figsize = (10,15))
    ...
    for agent_type in ['SS_New2', 'Functional', 'Agent', 'HERON']:
        if agent_type == 'SS_New2':
            ...
            profit, ppd, monthly_normalised_profit, nppd_month = getprofit(vals_dict,errors,agent_type,month)

            #color = 'tab:red'
            ax1[0].set_xlabel('time / days')
            ax1[0].set_ylabel('DNI / W/m$^2$')
            ax1[0].plot(vals_dict['times'],vals_dict['DNI'])
            ax1[0].tick_params(axis='y')

            #color = 'tab:red'
            ax1[1].set_xlabel('time / days')
            ax1[1].set_ylabel('Realtive Price / $/MWh')
            ax1[1].plot(vals_dict['times'],vals_dict['price'])
            ax1[1].tick_params(axis='y')

            ax1[2].set_title('Low Load Follow \n'f'Profit = {profit:.4} USD,' +f'  Profit per day = {ppd:.4} USD, Penalties and OOB Events = {errors}',fontsize=10)
                             
            ax1[2].set_xlabel('time / days')
            ax1[2].set_ylabel('Power Production / MW')
            ax1[2].plot(vals_dict['times'],vals_dict['power'], alpha=0.6, label = 'Power')
            ax1[2].tick_params(axis='y')
            for time in vals_dict[f'times_errors']:
                #print(time)
                ax1[2].axvspan(time, time+(1/24), color='orange', alpha=0.2)


            ax1[2].axvspan(0, 0, color='orange', alpha=0.2, label = 'Outside Bounds')

            ax2 = ax1[2].twinx()  # instantiate a second Axes that shares the same x-axis

            color = 'tab:red'
            ax2.set_ylabel('Concrete Temperature / $^\circ$C')  # we already handled the x-label with ax1
            ...
            ax1[3].set_title('High Load Follow \n'f'Profit = {profit:.4} USD,' +f'  Profit per day = {ppd:.4} USD, Penalties and OOB Events = {errors}',fontsize=10)
                             
            ax1[3].set_xlabel('time / days')
            ax1[3].set_ylabel('Power Production / MW')
            ax1[3].plot(vals_dict['times'],vals_dict['power'], alpha=0.6, label = 'Power')
            ax1[3].tick_params(axis='y')
            for time in vals_dict[f'times_errors']:
                #print(time)
                ax1[3].axvspan(time, time+(1/24), color='orange', alpha=0.2)


            ax1[3].axvspan(0, 0, color='orange', alpha=0.2, label = 'Outside Bounds')

            ax2 = ax1[3].twinx()  # instantiate a second Axes that shares the same x-axis
            ...
            ax1[4].set_title('DRL Agent \n'f'Profit = {profit:.4} USD,' +f'  Profit per day = {ppd:.4} USD, Penalties and OOB Events = {errors}',fontsize=10)

            ax1[4].set_xlabel('time / days')
            ax1[4].set_ylabel('Power Production / MW')
            ax1[4].plot(vals_dict['times'],vals_dict['power'], alpha=0.6, label = 'Power')
            ax1[4].tick_params(axis='y')
            for time in vals_dict[f'times_errors']:
                #print(time)
                ax1[4].axvspan(time, time+(1/24), color='orange', alpha=0.2)


            ax1[4].axvspan(0, 0, color='orange', alpha=0.2, label = 'Outside Bounds')

            ax2 = ax1[4].twinx()  # instantiate a second Axes that shares the same x-axis
            ...
            ax1[5].set_title('MILP Dispatch\n'f'Profit = {profit:.4} USD,' +f'  Profit per day = {ppd:.4} USD, Penalties and OOB Events = {errors}',fontsize=10)
                             
            ax1[5].set_xlabel('time / days')
            ax1[5].set_ylabel('Power Production / MW')
            ax1[5].plot(vals_dict['times'],vals_dict['power'], alpha=0.6, label = 'Power')
            ax1[5].tick_params(axis='y')
            for time in vals_dict[f'times_errors']:
                #print(time)
                ax1[5].axvspan(time, time+(1/24), color='orange', alpha=0.2)


            ax1[5].axvspan(0, 0, color='orange', alpha=0.2, label = 'Outside Bounds')

            ax2 = ax1[5].twinx()  # instantiate a second Axes that shares the same x-axis
        ...
        fig.tight_layout(h_pad=1)  # otherwise the right y-label is slightly clipped
        """ax1[2].legend(loc='lower left')
        ax1[3].legend(loc='lower left')
        ax1[4].legend(loc='lower left')
        ax1[5].legend(loc='lower left')
        ...
        fig.suptitle(f'Period of 30 days: {month+1}',fontsize=16, fontweight='bold' )
        ...
        plt.savefig(f'/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/PaperPlots/DispatchPlot_days_{month}.png')
        
        print('Agent = ' + agent_type + f', Month = {month}, Monthly Profit Nomalised = {monthly_normalised_profit}, Normalised Profit per Day = {nppd_month}, Penalties and OOB Events = {errors}')
