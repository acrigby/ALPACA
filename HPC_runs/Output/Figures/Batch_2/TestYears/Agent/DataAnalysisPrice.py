...
def read_csv_and_create_lists(path):
    ...
    with open(file_name, mode='r') as file:
        ...
        for row in reader:
            price_value = float(row[5])  # Column 4 (index 3)
            if price_value > max_val:
                max_val = price_value
            if price_value < min_val:
                min_val = price_value
    ...
    with open(file_name, mode='r') as file:
        ...
        for row in reader:
            #print(row)
            if float(row[0]) <10103628000:
            #    continue  # Skip rows with insufficient columns

                demand_value = (float(row[1])*10)+53.35  # Column 2 (index 1)
                price_value = float(row[5])  # Column 3 (index 2)
                ...
                segment_index = int(((price_value-min_val)/(max_val-min_val)) // (1 / 6))  # Assuming the value in column 3 ranges from 0 to 100
...
if __name__ == "__main__":
    ...
    for i, segment in enumerate(lists):
        ...
        try:
            ...
            sns.regplot(x = x_new, y = list(y), ci=100)
            plt.xlabel('Price / $/MWh')
            plt.ylabel('Power Demand / MW')
            #plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/FinalPPO3/Correlation{i}.png")
            plt.savefig(f"/home/rigbac/Projects/ALPACA/HPC_runs/Output/Figures/TestYears/Agent/Correlation_Price_{i}.png")
            plt.close()
