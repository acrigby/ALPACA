...
from PriceReader import importdict, importDNI
...
class ReactorEcomEnv(Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        self.clock = None
        self.isopen = True
        self.price_offset = random.randint(0, 404000)
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.price_schedule,self.prices = importdict('/home/rigbac/Projects/ALPACA/HPC_runs/Input/20240522-20240528 CAISO Day-Ahead Price','SCE',0) #start the function with the name of the file
        ...
        yout = getState(response,self.price_schedule,self.prices, self.t, self.price_offset)
    ...
    def step(self, action: np.ndarray):
        ...
        for i in range(len(price_schedule)):
            if price_schedule[i] <= self.t + self.price_offset < price_schedule[i+1]:
    ...
    def _get_ob(self):
        ...
        #self.variables = ["Time","BOP.sensorW.W","PowerDemand.y","DNI_Input.y[1]","CosEff_Input.y[1]", "BOP.deaerator.medium.p","dual_Pipe_CTES_Controlled_Feedwater.CTES.T_Ave_Conc","BOP.sensor_T2.T","SMR_Taveprogram.core.Q_total.y" ]
        return np.array(
            [(s[0]-53.35e6)/1e7, (s[1]-53.35e6)/1e7, (s[2]-500)/1000, s[3]-0.5, (s[4]-202000)/202000, (s[5]-423)/100, (s[6]-373)/100, (s[7]-1.58E8)/1E8, s[8]/50, s[9]/50, s[10]/50], dtype=np.float32
        )
...
def getState(response, price_schedule, prices, t , price_offset):

    for i in range(len(price_schedule)):
        if price_schedule[i] <= t + price_offset < price_schedule[i+1]:
