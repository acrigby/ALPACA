...
class ReactorEcomEnv(Env):
    ...
    def step(self, action: np.ndarray):
        ...
        if not terminated:
            reward = (reward_price/1e8)*average_power*2 + (1 - ((ns[3]-453)**2)/10)
            if p_penalty:
                reward -= 1
            if t_penalty:
                reward -= 1
    ...
    def _get_ob(self):
        ...
        #self.variables = ["Time","BOP.sensorW.W","PowerDemand.y","DNI_Input.y[1]","CosEff_Input.y[1]", "BOP.deaerator.medium.p","dual_Pipe_CTES_Controlled_Feedwater.CTES.T_Ave_Conc","BOP.sensor_T2.T","SMR_Taveprogram.core.Q_total.y" ]
        return np.array(
            [(s[0]-53.35e6)/1e7, (s[1]-53.35e6)/1e7, (s[2]-202000)/202000, (s[3]-423)/100, (s[4]-373)/100, s[5]/(self.price_max+1), s[6]/(self.price_max+1)], dtype=np.float32
        )
...
def getState(response, price_schedule, prices, t):
    ...
    state = [Power, PowerDemand, Deaer_P, Conc_Temp, fwit, reward_price_t, reward_price_t1]
