...
class ReactorEcomEnv(Env):
    ...
    def step(self, action):
        ...
        if not terminated:
            reward = ((reward_price/1e6)*average_power_reward*2) + (4 - ((ns[2]-441.5)**2))
            print(f"Reward based on Price = {((reward_price/1e6)*average_power_reward*2)}") 
            print(f"Reward based on Temperature = {(4 - ((ns[2]-441.5)**2))}")
            if p_penalty:
                reward -= 20
            if c_penalty:
                reward -= 20
    ...
    def _get_ob(self):
        ...
        return np.array(
            [(s[0]-53.35e6)/1e7, (s[1]-202000)/202000, (s[2]-441)/6, (s[3]-373)/100, s[4]/(1350),s[5]/(1350),s[6]/(1350),s[7]/2000, s[8]/2000], dtype=np.float32
        )
