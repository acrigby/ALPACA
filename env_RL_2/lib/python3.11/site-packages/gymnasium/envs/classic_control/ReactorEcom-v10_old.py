...
class ReactorEcomEnv(Env):
    ...
    os.system('cp -a /home/rigbac/Projects/ALPACA/HPC_runs/DymolaTemplatesFWHPT_DNI/. /scratch/rigbac/ThreadFilesFWHPT/' + str(tid))
    ...
    def step(self, action: np.ndarray):
        ...
        DeltaPowerDemand = min(max(action[0], self.min_action), self.max_action)
        ...
        if not terminated:
            reward = (reward_price/1e8)*average_power*2 + (1 - ((ns[2]-441)**2)/10)
            if p_penalty:
                reward -= 1
            if t_penalty:
                reward -= 1
    ...
    def _get_ob(self):
        ...
        return np.array(
            [(s[0]-53.35e6)/1e7, (s[1]-202000)/202000, (s[2]-441)/100, (s[3]-373)/100, s[4]/(self.price_max+1)], dtype=np.float32
        )
