...
class ReactorEcomEnv(Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        self.min_action = np.array([-8, 1000], dtype=np.float32)
    ...
    def step(self, action: np.ndarray):
        ...
        if not terminated:
            reward = (reward_price/1e8)*average_power - 0.2*abs(ns[3]-self.original_conc_temp)
            if p_penalty:
                reward -= 0.1
            if t_penalty:
                reward -= 0.1
    ...
    def _get_ob(self):
        ...
        #self.variables = ["Time","BOP.sensorW.W","PowerDemand.y","DNI_Input.y[1]","CosEff_Input.y[1]", "BOP.deaerator.medium.p","dual_Pipe_CTES_Controlled_Feedwater.CTES.T_Ave_Conc","BOP.sensor_T2.T","SMR_Taveprogram.core.Q_total.y" ]
        return np.array(
            [(s[0]-53.35e6)/1e7, (s[1]-53.35e6)/1e7, (s[2]-202000)/202000, (s[3]-423)/100, (s[4]-373)/100, (s[5]-1.58E8)/1E8, s[6]/10000, s[7]/10000, s[8]/10000], dtype=np.float32
        )
...
def getState(response, price_schedule, prices, t):
    ...
    state = [Power, PowerDemand, Deaer_P, Conc_Temp, fwit, CorePower,  reward_price_t, reward_price_t1, reward_price_t2]
