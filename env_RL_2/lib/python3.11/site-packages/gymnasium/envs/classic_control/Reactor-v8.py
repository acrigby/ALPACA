...
class ReactorEnv(Env):
    ...
    os.mkdir('/scratch/rigbac/ThreadFiles/' + str(tid))

    #Initialise the model and create first dsfinal file
    os.system('cp -a /home/rigbac/Projects/ALPACA/HPC_runs/DymolaTemplates/. /scratch/rigbac/ThreadFiles/' + str(tid))
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        os.chdir('/scratch/rigbac/ThreadFiles/' + str(tid))
        ...
        Tout = response["BOP.sensor_pT.T"][-1]
        PumpMFlow = response["pump_SimpleMassFlow1.m_flow"][-1]
        Qdemand = 40000000
        FF = response["FeedForward.y"][-1]
        ...
        oriInputFiles = ["start.txt"]
        currentInputFiles = ["dsfinal.txt"]

        changedvars = {'ramp.height':height} 
    ...
    def step(self, action: np.ndarray):
        ...
        if self.t == 9960:
            ...
            t_back = self.t-self.dt


        if self.t == 9960:
            deldelQ = (self.y.get(self.t+self.dt) - 2*self.y.get(self.t) + self.y.get(9960))/(self.dt*self.dt)
            delQ = self.y.get(self.t+self.dt) - self.y.get(9960)
        else:
            deldelQ = (self.y.get(self.t+self.dt) - 2*self.y.get(self.t) + self.y.get(self.t-self.dt))/(self.dt*self.dt)
            delQ = self.y.get(self.t+self.dt) - self.y.get(self.t-self.dt)

        print(self.t,delQ,deldelQ, FF, self.store_FF)
        ...
        ns, results, terminal = DymolaDyn(self,s_augmented, self.t, self.variables, self.results, self.fflist)
        ...
        if abs(deldelQ)<1000 and abs(delQ)<1000:
            if abs(FF - self.store_FF) > 0.1:
                print(abs(FF - self.store_FF))
                if 672.15 < s[0] < 674.15:
                    print(s[0])
                    reward -= math.pow(FF, 2)*20
...
def DymolaDyn(self, y0 , t,  variables, results, fflist):
    """
    ...
    for i in range(0,len(variables),1):
        results[variables[i]].extend(response[self.variables[i]])
