...
class ReactorEcomEnv(Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        self.min_action = np.array([-2, 1000], dtype=np.float32)
        self.max_action = np.array([2, 2500], dtype=np.float32)
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.original_conc_temp = yout[3]
    ...
    def step(self, action: np.ndarray):
        ...
        ns, terminal, p_penalty, t_penalty = DymolaDyn(self, s_augmented)
        ...
        for i in range(len(self.price_schedule)):
            if self.price_schedule[i] <= self.t  < self.price_schedule[i+1]:
                reward_price = self.prices[i]/self.price_average
        ...
        if not terminated:
            reward = (reward_price/1e8)*average_power - 0.2*abs(ns[3]-self.original_conc_temp) - 0.25*abs(ns[4]- (148+273.15)) - (2/101000)*abs(ns[2]- 133000)
            steps += 1
            print(reward)
        elif self.steps_beyond_terminated is None:
            # Pole just fell!
            self.steps_beyond_terminated = 0
            print(10000*(ns[3]-self.original_conc_temp))
            reward = (reward_price/1e8)*average_power
    ...
    def _get_ob(self):
        ...
        #self.variables = ["Time","BOP.sensorW.W","PowerDemand.y","DNI_Input.y[1]","CosEff_Input.y[1]", "BOP.deaerator.medium.p","dual_Pipe_CTES_Controlled_Feedwater.CTES.T_Ave_Conc","BOP.sensor_T2.T","SMR_Taveprogram.core.Q_total.y" ]
        return np.array(
            [(s[0]-53.35e6)/1e7, (s[1]-53.35e6)/1e7, (s[2]-202000)/202000, (s[3]-423)/100, (s[4]-373)/100, s[5]/10000, s[6]/10000, s[7]/10000], dtype=np.float32
        )
...
def getState(response, price_schedule, prices, t):
    ...
    state = [Power, PowerDemand, Deaer_P, Conc_Temp, fwit, reward_price_t, reward_price_t1, reward_price_t2]
