...
class ReactorEcomEnv(Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        print('Copying over start conditions')

        os.makedirs('/scratch/rigbac/ThreadFilesFWHPT/' + str(tid), exist_ok=True)

        os.system('cp -a -f /home/rigbac/Projects/ALPACA/HPC_runs/DymolaTemplatesFWHPT_DNI_2/. /scratch/rigbac/ThreadFilesFWHPT/' + str(tid))
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if options is not None:
            start_offset = options['month']*720+ options['period']*120
    ...
    def step(self, action):
        ...
        try:
            ns, terminal, p_penalty, t_penalty, c_penalty = DymolaDyn(self, s_augmented, reward_price_t, reward_price_t1, reward_price_t2, prevDNI, DNI, prevIAM, IAM)
        except:
            terminal = True
            ns = self.state
            p_penalty = True
        ...
        if not terminated:
            reward = (reward_price/1e8)*average_power*3 + (3 - ((ns[2]-441.5)**2))
            if p_penalty:
                reward -= 2
            if t_penalty:
                reward -= 2
    ...
    def _get_ob(self):
        ...
        return np.array(
            [(s[0]-53.35e6)/1e7, (s[1]-202000)/202000, (s[2]-441)/10, (s[3]-373)/100, s[4]/(self.price_max+1),s[5]/(self.price_max+1),s[6]/(self.price_max+1),s[7]/2000], dtype=np.float32
        )
...
def DymolaDyn(self, y0, reward_price_t, reward_price_t1, reward_price_t2, prevDNI, DNI, prevIAM, IAM):
    """
    ...
    for value in response["BOP.deaerator.medium.p"]:
        if value < 111000 or value > 201000:
            p_penalty = True
...
def getState(response, reward_price_t, reward_price_t1, reward_price_t2, DNI):
    ...
    state = [PowerDemand, Deaer_P, Conc_Temp, fwit, reward_price_t, reward_price_t1, reward_price_t2, DNI]
